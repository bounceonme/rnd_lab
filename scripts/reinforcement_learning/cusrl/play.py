...
def _keyboard_sensitivity(axis_range: tuple[float, float]) -> float:
    return max(abs(axis_range[0]), abs(axis_range[1]))
...
class CameraFollowPlayerHook(cusrl.Player.Hook):
    ...
    def step(self, step: int, transition: dict, metrics: dict):
        camera_follow(self.player.environment, task_name=self.task_name)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: TrainerCfg):
    ...
    if args_cli.keyboard:
        ...
        config = Se2KeyboardCfg(
            v_x_sensitivity=_keyboard_sensitivity(env_cfg.commands.base_velocity.ranges.lin_vel_y)
            if "RND-Step" in args_cli.task
            else _keyboard_sensitivity(env_cfg.commands.base_velocity.ranges.lin_vel_x),
            v_y_sensitivity=_keyboard_sensitivity(env_cfg.commands.base_velocity.ranges.lin_vel_x)
            if "RND-Step" in args_cli.task
            else _keyboard_sensitivity(env_cfg.commands.base_velocity.ranges.lin_vel_y),
            omega_z_sensitivity=_keyboard_sensitivity(env_cfg.commands.base_velocity.ranges.ang_vel_z),
        )
        controller = Se2Keyboard(config)
        env_cfg.observations.policy.velocity_commands = ObsTerm(
            func=lambda env: _keyboard_command_for_task(
                args_cli.task, torch.tensor(controller.advance(), dtype=torch.float32, device=env.device).unsqueeze(0)
            ),
        )
    ...
    if args_cli.keyboard:
        camera_follow(player.environment, task_name=args_cli.task)
        player.register_hook(CameraFollowPlayerHook(task_name=args_cli.task))
